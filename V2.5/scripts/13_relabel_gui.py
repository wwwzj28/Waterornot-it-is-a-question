...
可选 --filter effective_lowmed 只看当前有效标签是 low/medium 的样本（relabel 优先）。
"""
...
def effective_label(image_name: str, store: dict[str, dict]) -> str:
    rec = store.get(image_name, {})
    new_label = (rec.get("new_label", "") or "").strip().lower()
    if new_label and new_label != "skip":
        return new_label
    return load_raw_label(JSON_DIR / f"{Path(image_name).stem}.json")
    ...
    if filter_name == "only_lowmed":
        ...
        for p in files:
            ...
            if old in {"low", "medium"}:
                keep.append(p)
        return keep
    if filter_name == "effective_lowmed":
        store = load_existing_labels(RELABEL_CSV)
        return [p for p in files if effective_label(p.name, store) in {"low", "medium"}]
...
def main():
    ...
    parser.add_argument("--filter", choices=["all", "only_lowmed", "effective_lowmed", "unlabeled"], default="all")
