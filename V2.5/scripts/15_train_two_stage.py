...
from torchvision.models import EfficientNet_V2_S_Weights, ResNet50_Weights, efficientnet_v2_s, resnet50
...
def train_log_path(backbone: str, run_name: str | None = None) -> Path:
    if backbone == "resnet50" and not run_name:
        return METRICS_DIR / "two_stage_train_log.csv"
    suffix = backbone_output_name(backbone, run_name)
    return METRICS_DIR / f"two_stage_train_log_{suffix}.csv"


def make_transforms(use_stage_b_aug: bool = False, input_h: int = INPUT_H, input_w: int = INPUT_W):
    train_steps = [
    ...
    if use_stage_b_aug:
        train_steps.extend([
            ...
            transforms.RandomResizedCrop((input_h, input_w), scale=(0.88, 1.0)),
        ])
    train_steps.extend([
...
def build_loaders(label_to_idx, use_weighted_sampler: bool = False, use_stage_b_aug: bool = False, input_h: int = INPUT_H, input_w: int = INPUT_W):
    train_tf, eval_tf = make_transforms(use_stage_b_aug=use_stage_b_aug, input_h=input_h, input_w=input_w)
    ...
    sampler = make_weighted_sampler(ds_train.targets, num_classes) if use_weighted_sampler else None
    return {
        "train": DataLoader(ds_train, batch_size=BATCH_SIZE, shuffle=sampler is None, sampler=sampler, num_workers=0),
        "val": DataLoader(ds_val, batch_size=BATCH_SIZE, shuffle=False, num_workers=0),
    }, {"train": len(ds_train), "val": len(ds_val)}, ds_train


def make_model(num_classes, backbone: str):
...
def classifier_parameters(m, backbone: str):
    return m.fc.parameters() if backbone == "resnet50" else m.classifier.parameters()


def freeze_backbone(m, backbone: str):
    for p in m.parameters():
        p.requires_grad = False
    for p in classifier_parameters(m, backbone):
...
def train_one_stage(
    ...
    loaders, sizes, ds_train = build_loaders(
        label_to_idx,
        use_weighted_sampler=use_weighted_sampler,
        use_stage_b_aug=use_stage_b_aug,
        input_h=input_h,
        input_w=input_w,
    )
    ...
    if use_stage_b_aug:
        print("train augmentation: Stage B light affine/crop")
    ...
    opt = optim.AdamW(classifier_parameters(model, backbone), lr=WARMUP_LR, weight_decay=WEIGHT_DECAY)
...
def main():
    ...
    print(f"input_size = {args.input_h}x{args.input_w}")

    out_dir = MODEL_DIR / backbone_output_name(args.backbone, args.run_name) / "two_stage"
    out_dir.mkdir(parents=True, exist_ok=True)
    log_csv = train_log_path(args.backbone, args.run_name)
    ...
    with log_csv.open("w", encoding="utf-8-sig", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["stage", "epoch", "phase", "loss", "acc", "lr"])
        writer.writeheader()
